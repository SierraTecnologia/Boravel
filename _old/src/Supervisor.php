...
class Supervisor implements Pausable, Restartable, Terminable
{
    ...
    public function __construct(SupervisorOptions $options)
    {
        ...
        app(BossCommandQueue::class)->flush($this->name);
    }
    ...
    {
        return ! config('boss.fast_termination') ||
               app(CacheFactory::class)->get('boss:terminate:wait');
    }
    ...
    {
        foreach (app(BossCommandQueue::class)->pending($this->name) as $command) {
