...
class RedisJobRepository implements JobRepository
{
    ...
    {
        $this->redis = $redis;
        $this->recentJobExpires = config('boss.trim.recent', 60);
        $this->failedJobExpires = config('boss.trim.failed', 10080);
        $this->monitoredJobExpires = config('boss.trim.monitored', 10080);
    }
    ...
    {
        return $this->redis->connection('boss');
    }
}
