...
use Boss\Events\RelationAttached;
use Boss\Events\RelationAttaching;
use Boss\Events\RelationDetached;
use Boss\Events\RelationDetaching;
use Boss\Events\RelationToggled;
use Boss\Events\RelationToggling;
use stdClass;

/**
 * Class Boss da Biblioteca Follow.
 */
class BossByFollow
{
    use SoftDeletes;
    ...
    {
        $target = self::formatTargets($target, $class ?: config('boss.user_model'));
    ...
    public static function attachPivotsFromRelation(MorphToMany $morph, $targets, $class)
    {
        return self::formatTargets($targets, $class, [
            ...
            'created_at' => Carbon::now()->format(config('boss.date_format', 'Y-m-d H:i:s')),
        ]);
    }
    ...
    {
        return \sprintf('%s.%s', config('boss.bossable_table'), $field);
    }
}
