...
class LongWaitDetected extends Notification
{
    ...
    {
        return array_filter([
            Boss::$slackWebhookUrl ? 'slack' : null,
            Boss::$smsNumber ? 'nexmo' : null,
            Boss::$email ? 'mail' : null,
        ]);
    }
    ...
    public function toSlack($notifiable)
    {
        return (new SlackMessage)
                    ->from('Laravel Boss')
                    ->to(Boss::$slackChannel)
                    ->image('https://laravel.com/assets/img/boss-48px.png')
                    ->error()
