...
class AccountService
{
    ...
    {
        $accessCollection = $this->config['plans'][$this->subscription->sitecpayment_plan]['access'];
    ...
    {
        return $this->config['plans'][$this->subscription->sitecpayment_plan][$key];
    }
    ...
    {
        return $this->config['plans'][$this->subscription->sitecpayment_plan]['limits'][$model];
    }
    ...
    public function withinLimit($model, $key = 'user_id', $value = null)
    {
        ...
        $limit = $this->config['plans'][$this->subscription->sitecpayment_plan]['limits'][$model];
        ...
        if ($this->inBillingCycle) {
            ...
            $sitecpaymentSubscription = $customer->subscriptions->retrieve($this->subscription->stripe_id);

            $currentPeriodStart = Carbon::createFromTimestamp($sitecpaymentSubscription->current_period_start)->format('Y-m-d h:i:s');
            $currentPeriodEnd = Carbon::createFromTimestamp($sitecpaymentSubscription->current_period_end)->format('Y-m-d h:i:s');
    ...
    public function creditsUsed($model, $key = 'user_id', $value = null)
    {
        ...
        $credits = $this->config['plans'][$this->subscription->sitecpayment_plan]['credits'];
    ...
    public function clause($key, $method, $model = null)
    {
        ...
        $clause = $this->config['plans'][$this->subscription->sitecpayment_plan][$key];
