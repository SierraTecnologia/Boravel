...
class BoardController extends BaseController
{
    ...
    {
        return [
            \App\Logic\Boards\Views\Computer::class,
            \App\Logic\Boards\Views\File::class
        ];
    }
    ...
    {
        $boards = [];
        return view('components.boards.index', compact('boards'));
    }

    public function show()
    {
        $boardName = $this->getBoards()[0];
        $navegador = new NavegadorComponent();

        $board = new $boardName($navegador);
        
        return view('components.boards.board', compact('board'));
        
        /**
        * $entity = new File(false, '<?xml version="1.0" encoding="UTF-8"?>
